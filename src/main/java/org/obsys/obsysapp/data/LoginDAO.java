...
public class LoginDAO {
    ...
    /**
     * Queries the database for user login data by username. Values are
     * defaulted to account for null username results.
     *
     * @param conn     A valid connection to the database.
     * @param username The username to be queried.
     * @return The Person ID(PK), password, and Admin status of the queried
     * username.
     * @throws SQLException Indicates a failed query attempt.
     */
    public Login readPasswordByUsername(
        ...
        try (PreparedStatement statement = conn.prepareStatement("""
                SELECT UserPassword, IsAdmin, PersonId
                FROM dbo.Person WHERE Username = ?
                    """)) {
    ...
    /**
     * Queries the database for a username and returns teh number of that
     * username found. The database cannot support uniqueness and NULL values.
     * This statement is used to check for existing username to enforce
